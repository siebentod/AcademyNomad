...
export function useHighlights() {
  ...
  const setHighlightsTotalCount = useStore(
    (state) => state.setHighlightsTotalCount
  );
  ...
  useEffect(() => {
    const processHighlights = async () => {
      ...
      if (activeList?.name) {
        const { items, total: highlightsTotalCountValue } = await getHighlights(
          {
        ...
        newHighlights = items || [];
        setHighlightsTotalCount(highlightsTotalCountValue);
        setTotalFiltered(highlightsTotalCountValue);
      } else {
        const { items, total: highlightsTotalCountValue } = await getHighlights(
          {
            limit: visibleHighlightsCount,
