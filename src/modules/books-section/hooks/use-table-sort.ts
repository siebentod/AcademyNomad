...
export const useTableSort = (files: File[], defaultSort: string | null, activeListName: string) => {
  ...
  const sortedFiles = useMemo(() => {
    ...
    return [...files].sort((a, b) => {
      // Сначала сортируем по is_pinned (true вверху) только если есть activeListName
      if (activeListName) {
        ...
        // Затем по pinned_order (от меньшего к большему) только если есть activeListName
  ...
  }, [files, sortColumn, sortDirection, defaultSort, activeListName]);
